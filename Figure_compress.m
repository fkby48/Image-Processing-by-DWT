...
switch handles.wavetype
    case 1
        [C,S]=wavedec2(handles.original,handles.n,'haar');
        CA1=appcoef2(C,S,'haar',1); %尺度1的低频系数
        CA2=appcoef2(C,S,'haar',2); %尺度2的低频系数
    case 2
        [C,S]=wavedec2(handles.original,handles.n,'db4');
        CA1=appcoef2(C,S,'db4',1);
        CA2=appcoef2(C,S,'db4',2);
    case 3
        [C,S]=wavedec2(handles.original,handles.n,'sym4');
        CA1=appcoef2(C,S,'sym4',1);
        CA2=appcoef2(C,S,'sym4',2);
end
percentage=0.3;waitbar(percentage,h_waitbar,['计算中......',num2str(percentage*100),'%']);
%第一层
...
title('第一次压缩图像');
percentage=0.6;waitbar(percentage,h_waitbar,['计算中......',num2str(percentage*100),'%']);
%第二层
...
title('第二次压缩图像');
percentage=0.9;waitbar(percentage,h_waitbar,['计算中......',num2str(percentage*100),'%']);
